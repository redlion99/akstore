...
import me.smartco.akstore.biz.service.{TransactionService, PartnerService}
    ...
    pathPrefix("partners" / Segment) { partnerId =>
      ...
      authorize(user.hasRole("Staff") || partnerId.equals(partnerStaff.getPartner.getId)) {
        ...
        path("account"/"balance"){
          ...
          path("orders") {
            val itemRepo = partnerManager.getLineItemDAO
            parameters('shop_id.?, 'dateStart.?, 'dateEnd.?, 'page.as[Int]) { (shop, dateStart, dateEnd, page) =>
              ...
              complete {
                ...
                shop match {
                  case Some(shop_id) =>
                    partnerManager.searchOrder(shopRepo.findOne(shop_id), start, end, AbstractDocument.pageRequest(page))
                  case None =>
                    partnerManager.getOrderDAO.findByPartnerId(partner.getId, AbstractDocument.pageRequest(page))
                }
          ...
          path("orders" / Segment) { orderId =>
            get {
              complete {
                partnerManager.getOrderDAO.findOne(orderId).toJson
              }
            }
          } ~
          path("orders" / Segment/"status") { orderId =>
            post {
              formFields('_status.as[Int]){
                (status) =>
                  complete {
                    val order=partnerManager.getOrderDAO.findOne(orderId)
                    if(OrderStatus.hasFlag(status,order.getStatus)){
                      order.setStatus(status)
                      partnerManager.createOrderHistory(order)
                      partnerManager.getOrderDAO.save(order).toBriefJson
                    }
                    order.toBriefJson
                  }
              }
          ...
          path("bills") {
            val billRepo = partnerManager.getBillRepository
          ...
          path("billflows") {
            val billflowRepo = partnerManager.getBillFlowRepository
